...
| `src/lib/settings/` | Reactive prefs stores (units, maps, messages-preferences, theme, ui-scale, `log-prefs` — device-local APRS Logs auto-refresh/auto-scroll toggles, GH #373; `notification-prefs` — device-local toast/OS/both notification mode, see [notifications.md](notifications.md)) |
