...
| `stationcache` | Heard-station cache (memory + persistent) and APRS-extract helpers. Each `Station`/`CacheEntry` carries `StatusCode`/`StatusText` (Mic-E message code, APRS101 ch 10 table 8, or a `'>'` status report's free text) -- `-1` is the "no status" sentinel, since wire code `0` means Emergency and would otherwise collide with Go's int zero value (every construction path -- `buildStationEntry`, `buildObjectEntry`, `MemCache.Update`'s new-station branch, `historydb.LoadRecent`'s hydration -- sets it explicitly). `GET /api/stations` (`pkg/webapi/stations.go`) exposes both fields on `StationDTO` (`status_code` deliberately not `omitempty`, for the same zero-value reason); `GET /api/stations/alerts` returns just the subset currently at `StatusCode == 0` (Emergency), regardless of map viewport, for `stationAlertsTransport.js` (see [notifications.md](notifications.md)). | (no dedicated page) |
...
| `src/routes/` | One Svelte route per page (Dashboard, LiveMapV2, Channels, Beacons, Digipeater, Igate, Kiss, Agw, Ptt, Gps, AudioDevices, Messages, **Bulletins**, Terminal + TerminalTranscripts, PositionLog, MapsSettings, Preferences (`/preferences`, "General" nav label), MessagesSettings, **NotificationsSettings** (`/preferences/notifications` — popup mode + per-type message/bulletin sounds, see [notifications.md](notifications.md)), Login, About, Logs, Simulation) |
