...
| Mic-E | Compact APRS encoding for mobile rigs; also carries a 3-bit status/message code (APRS101 ch 10 table 8) in the destination callsign's message bits -- Off Duty/En Route/In Service/Returning/Committed/Special/Priority/Emergency. Decode: `pkg/aprs/mice.go`'s `miceMessageLabels` (wire code 0 = Emergency, 7 = Off Duty -- inverted vs. intuition, see the comment there). Outbound: the operator picks their own beacon's code via the Beacons page's Status select (only shown for `position_format: mic_e`); stored as a named string enum (`configstore.Beacon.MicEMessageCode`, e.g. `"off_duty"`), not the raw wire int, specifically to avoid a zero-value/Emergency collision -- see `pkg/beacon/mice.go`'s `micEMessageCodesByName`. Picking Priority/Emergency requires an explicit confirm dialog (`web/src/routes/Beacons.svelte`) since it alarms other operators' radios. | [`../../pkg/aprs/mice.go`](../../pkg/aprs/mice.go), [`../../pkg/beacon/mice.go`](../../pkg/beacon/mice.go) |
| Status report (`'>'`) | APRS101 ch 16 free-form status text, positionless. Decoded into `DecodedAPRSPacket.Status`; surfaced in the packet log/inspector (`pkg/webapi/packets.go`) and, for an already-known station, updates `stationcache`'s per-station `StatusText` without disturbing its plotted position (`pkg/stationcache/extract.go`'s positionless branch in `ExtractEntry`). | [`../../pkg/aprs/parse.go`](../../pkg/aprs/parse.go) |
