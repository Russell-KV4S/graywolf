...
bulletins, and station Emergency status, plus the unread-badge lag fix
that motivated the same change.
...
| Per-type master switch | Same store — `messageEnabled`/`bulletinEnabled`/`stationEmergencyEnabled` (`setMessageEnabled`/`setBulletinEnabled`/`setStationEmergencyEnabled`), all **on by default**. Independent of `mode`: gates whether a type notifies at all (popup + OS + sound), ahead of the mute/active-thread/page-open suppression rules. Turning all three off is "notifications off"; turning off one mutes just that type. |
...
| Preferences UI | `web/src/routes/NotificationsSettings.svelte` at `/preferences/notifications` (own sidebar entry, not under General/`Preferences.svelte`) — "Popup notifications" `Box`: the three per-type toggles, a mode picker, plus **"Send test notification"** and **"Send test emergency notification"** buttons, usable by any operator (not a dev-only affordance), that fire a real sample through whichever mode is currently selected via `{force: true}` — so the operator can preview exactly what they'll get before relying on it. The emergency test button fetches `GET /api/station/config` on mount and deep-links to `#/map?focus=<the operator's own callsign>` (falling back to a plain `#/map` link if unset) — the exact same href shape a real Emergency alert uses — so clicking it exercises the real click-through (camera fly-to, popup open, the popup's Message/Logs/QRZ action row) without a real Emergency packet needing to go out over RF to test it. |

### Notification sounds (per-type, device-local, custom upload)

Separate sound settings for messages, bulletins, and station
emergencies, alongside the mode picker in `NotificationsSettings.svelte`'s
"Message sounds" / "Bulletin sounds" / "Station emergency sounds" boxes.
Device-local like the toast/OS mode above (see
